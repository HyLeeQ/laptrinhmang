...
public class FilesViewController {
    ...
    private void loadFiles() {
        if (messages == null || filesListView == null)
            return;
    ...
    private void downloadOrOpenFile(FileItem fileItem) {
        ...
        if (message == null) {
            logger.error("Message is null for FileItem");
        ...
        logger.info("Attempting to download/open file: {} (messageId: {})", fileName, messageId);

        // Nếu file đã có data trong message, mở trực tiếp
        if (message.getFileData() != null && message.getFileData().length > 0) {
            logger.info("File data already available ({} bytes), opening directly", message.getFileData().length);
            openFileDirectly(fileName, message.getFileData());
            return;
        }

        // Nếu chưa có data, request từ server
        logger.info("File data not available, requesting from server for messageId: {}", messageId);
        ...
        ChatEventManager.getInstance().registerFileDownloadCallback(messageId, downloadInfo -> {
            logger.info("File download callback triggered for messageId: {}", messageId);
            if (downloadInfo != null && downloadInfo.getFileData() != null) {
                logger.info("File downloaded successfully: {} ({} bytes)",
                        downloadInfo.getFileName(), downloadInfo.getFileData().length);
                openFileDirectly(downloadInfo.getFileName(), downloadInfo.getFileData());
            } else {
                logger.error("File download failed for messageId: {}", messageId);
                Platform.runLater(() -> {
        ...
        logger.info("Sending GET_FILE request for messageId: {}", messageId);
        ChatClient.sendRequest("GET_FILE|" + messageId);
    }

    private void openFileDirectly(String fileName, byte[] fileData) {
        logger.info("openFileDirectly called for: {} ({} bytes)", fileName, fileData != null ? fileData.length : 0);

        if (fileData == null || fileData.length == 0) {
            logger.error("File data is null or empty for: {}", fileName);
            Platform.runLater(() -> showAlert("Dữ liệu file rỗng!"));
            return;
        }
        ...
        Platform.runLater(() -> {
            try {
                ...
                logger.info("File data written to temp file successfully");

                // Mở file
                try {
                    logger.info("Attempting to open file with Desktop.open()");
                    java.awt.Desktop.getDesktop().open(tempFile);
                    logger.info("File opened successfully");
                } catch (Exception e) {
                    logger.warn("Cannot open file with Desktop.open(), showing save dialog: {}", e.getMessage());
                    ...
                    if (selectedFile != null) {
                        ...
                        logger.info("File saved to: {}", selectedFile.getAbsolutePath());
                ...
                logger.error("Lỗi khi mở file: {}", e.getMessage(), e);
