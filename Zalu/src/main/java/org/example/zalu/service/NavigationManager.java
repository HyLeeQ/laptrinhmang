...
public class NavigationManager {
    ...
    public NavigationManager(ImageView navAvatarImage, UserDAO userDAO) {
    ...
    /**
     * Refresh menu header với thông tin user hiện tại
     */
    public void refreshMenuHeader(int currentUserId) {
        this.currentUserId = currentUserId;
        refreshMenuHeader();
    }
    
    private void refreshMenuHeader() {
        if (navMenuNameLabel == null || userDAO == null || currentUserId <= 0) return;
        try {
            User me = userDAO.getUserById(currentUserId);
            if (me != null) {
                String displayName = (me.getFullName() != null && !me.getFullName().isBlank()) 
                        ? me.getFullName() 
                        : me.getUsername();
            ...
            navMenuNameLabel.setText("Tài khoản của bạn");
        }
    }
     ...
     */
    public void updateAvatar(int currentUserId) {
        if (navAvatarImage == null || userDAO == null || currentUserId <= 0) {
            return;
        }
        ...
        Platform.runLater(() -> {
            try {
                User me = userDAO.getUserById(currentUserId);
                Image avatar = AvatarService.resolveAvatar(me);
                if (avatar != null) {
                    navAvatarImage.setImage(avatar);
                }
            } catch (Exception e) {
                System.err.println("Không thể tải avatar người dùng: " + e.getMessage());
